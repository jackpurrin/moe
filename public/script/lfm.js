...
const RelaxedComparator = new Intl.Collator(undefined, {
	sensitivity: 'base'
});
...
class Tracks extends HTMLElement {
	...
	connectedCallback() {
		...
		this.shadowRoot.appendChild(create('link', {
			rel: 'stylesheet',
			id: 'basestyles',
			href: basestyles.href
		}));
		this.#init();
	...
	#renderScrobbles(o) {
		...
		if (scrobbles?.length === 0) {
			...
			this.#scrobblesProcessor.process(scrobbles).getItems().forEach((item) => {
				if (item.type === 'track') {
					lines.push(create('div', {
							class: item.pinfo.text === 'playing' ? 'trackinfo nowplaying' : 'trackinfo'
						},
						create('a', {
								class: 'cover',
								href: item.albumUrl,
